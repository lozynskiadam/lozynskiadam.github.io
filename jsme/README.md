...
| `src/core/store.js` | Reaktywny stan UI + reguły edycji nad `mapData`. Każda mutacja mapy podbija `state.mapRevision`. |
...
| `src/core/menus.js` | Definicja paska menu jako lista id akcji. |
| `src/core/shortcuts.js` | Notacja skrótów klawiszowych: normalizacja, dopasowanie do `KeyboardEvent`, format do wyświetlenia. |
| `src/core/mapFile.js` | Otwieranie/zapis pliku mapy w przeglądarce + walidacja formatu. |
...
- **Nowa mutacja mapy** – funkcja w `store.js`, która po zmianie woła `touchFloor(z)`
  (lub `touchAll()`), i eksport w zwracanym obiekcie.
- **Nowy element HUD zależny od stanu** – dopisz pole do listy w `renderer.subscribe()`.

## Format pliku mapy

JSON: `{ [z]: { [y]: { [x]: [ { id, ...właściwości }, … ] } } }`. `id` to numer
itemu z katalogu; pozostałe klucze wpisu to własne właściwości ustawione w
dialogu „Properties” (klucze normalizowane do camelCase).
